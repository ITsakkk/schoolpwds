...
// Search
$search_by = isset($_GET['search_by']) ? $_GET['search_by'] : 'studentid';
...
if (!empty($search_term)) {
    $search_term = mysqli_real_escape_string($conn, $search_term); // Sanitize input
    $query .= " AND $search_by LIKE '%$search_term%'";
}
...
<body>
    ...
    <div class="content px-4">
            ...
            <div class="col-lg-3 py-2 p-0 text-start">
                <form class="form-inline" id="searchForm" action="display_student.php" method="GET">
                    <div class="form-group rounded border border-primary">
                        <select class="form-select mr-sm-2 border-0 outline-0" name="search_by" aria-label="Search by">
                            <option value="studentid" <?php if ($search_by == 'studentid') echo 'selected'; ?>>Student
                                ID</option>
                            <option value="nameinlatin" <?php if ($search_by == 'nameinlatin') echo 'selected'; ?>>Name
                                in Latin</option>
                            <option value="phonenumber" <?php if ($search_by == 'phonenumber') echo 'selected'; ?>>Phone
                                Number</option>
                            <option value="email" <?php if ($search_by == 'email') echo 'selected'; ?>>Email</option>
                        </select>
                        <input class="form-control border-0 outline-0" type="text" name="search_term"
        ...
        <div class="row d-flex align-items-center p-2 border DFDFDF <?php echo $bg_color_class; ?>">
            ...
            <div class="col-lg-2 font-weight-bold text-truncate"><?php echo $row['studentid'] ?></div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="seeMoreDropdown">
                        ...
                        <li><a class="dropdown-item text-danger"
                                href="update_status.php?studentid=<?php echo $row['studentid'] ?>"><i
                                    class="fa-solid fa-trash-can fs-sm-7"></i> Update Status</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            echo "<li class='page-item'><a class='page-link fs-5 p-2 px-3 text-primary bg-light border-primary' href='?page=" . $i . "&search_by=" . $search_by . "&search_term=" . $search_term . "'>" . $i . "</a></li>";
        }
        echo "</ul>";
        echo "</div>";
        ?>
